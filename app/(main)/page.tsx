      ...
      <main className="p-4 md:p-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {restaurants.map((r) => (
              <Card key={r.id} className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
                ...
                <CardContent>
                  <div className="flex justify-between items-center text-sm text-gray-600">
                    <div className="flex items-center gap-1">
                      <Clock size={16} />
                      <span>{r.delivery_time_minutes} min</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <DollarSign size={16} />
                      <span>Frete R$ {r.delivery_fee.toFixed(2)}</span>
                    </div>
                  </div>
                  <Badge variant="outline" className="mt-4">{r.category}</Badge>
                </CardContent>
              </Card>
            ))}
          </div>
